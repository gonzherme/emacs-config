...
(use-package easysession
  ...
  (defun my-home-open-session ()
    "Load session at point."
    (interactive)
    (let ((session (get-text-property (point) 'session-name)))
      (when session
        (easysession-switch-to session))))

  (defun my-home-new-session ()
    "Create a new session."
    (interactive)
    (call-interactively #'easysession-save))
  ...
  (defun my-home-refresh ()
    (let ((inhibit-read-only t))
      ...
      (insert (propertize
               "Gonz's EMACS\n"
               'face '(:height 3.0 :weight bold)))

      (insert "\n")
      (insert (propertize
               "Sessions\n\n"
               'face '(:height 1.4 :weight semi-bold)))

      ;; Session list
      (dolist (session (sort (easysession--get-all-names) #'string<))
        (let ((start (point)))
          (insert (format "  %s\n" session))
          (add-text-properties
           start
  ...
  ;; Keybindings for startup screen
  (define-key my-home-mode-map (kbd "RET") #'my-home-open-session)
  (define-key my-home-mode-map (kbd "<mouse-1>") #'my-home-open-session)
  (define-key my-home-mode-map (kbd "n") #'my-home-new-session)

(defun my-home-screen ()
  "Display startup session launcher."
  (delete-other-windows)   ;; <- add this
  (switch-to-buffer "Home")
  (my-home-mode)
  (my-home-refresh))

  ;; Show launcher on startup
  (add-hook 'emacs-startup-hook #'my-home-screen))
  #+end_src
...
** Emacs Screen Saver
*Zone*
- emacs built-in screen saver package
- screen saver function in =my-packages/letters.el=
- stopped using zone-matrix, eats up CPU
#+begin_src emacs-lisp
(use-package zone
  :ensure nil ;; built-in core package
  :config
  (require 'letters)
  (setq zone-delete-other-windows t) ;; Forces full-frame screen takeover
  ;; run my-packages/letters.el function on screen save
  (setq zone-programs [zone-pgm-letters-minimal])

  ;; on 10 mins of inactivity
  (zone-when-idle 600))
#+end_src
...
[[https://github.com/bbatsov/super-save][super-save]] not required, built-in good enough
#+begin_src emacs-lisp
;; (use-package super-save
;;   :config
;;   ;; saves when clicking away from Emacs, changing buffers, or moving windows
;;   (setq super-save-when-focus-lost t)  
;;   ;; silent mode: don't show "Wrote file..." on mini-buffer on every save
;;   (setq super-save-silent t)
;;   ;; ensure it saves org src code blocks
