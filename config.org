...
By default, GUI Emacs on macOS doesn't inherit the terminal's =$PATH=. 

The standard workaround is the =exec-path-from-shell= package, but it loads  an entire interactive shell on startup (including heavy themes, Oh My Zsh, etc), which delays Emacs booting by over a second. 

To speed up the startup sequence, we bypass the shell entirely and manually tell Emacs where to find our Homebrew and GNU utilities. This connects Emacs 
to tools like =gopls=, =fd=, and =coreutils= instantly.
...
* Startup Speed
Function to print out the duration of Emacs startup:
#+begin_src emacs-lisp
