...
;; Open dired ~/gonz/ on startup
(setq initial-buffer-choice "~/gonz/")
#+end_src
...
*** Auto Dark
*Custom theme tweaks*
Below are some tweaks to the themes being loaded, like removing fringes and changing the tab colors. These need to be reapplied when =auto-dark= switches themes.

This function which performs these tweaks runs on startup and =auto-dark='s theme switches
#+begin_src emacs-lisp
(defun my/apply-theme-tweaks ()  
  ...
  (let ((bg-main (face-background 'default nil t))
    ...
    ;; 2. Active Tab (Uses main background + your custom accent)
    ...
    ;; 3. Inactive Tabs (Matches inactive background to blend in)
...
#+begin_src emacs-lisp
;; Margin percentages
(defvar my-single-window-margin-pct 0.1 ;; 10%
  "Left margin as a fraction of window width when only one window exists.")

(defvar my-multi-window-margin-pct 0.05 ;; 5%
  "Left margin as a fraction of window width when multiple windows exist.")

;; function that adjusts margins dynamically
(defun my/update-window-margins (&optional _frame)
  "Set left margins as a percentage of each window's width."
  (let ((is-single (one-window-p t)))
    (walk-windows
     (lambda (win)
       (let* ((width (window-total-width win))
              (pct (if is-single
                       my-single-window-margin-pct
                     my-multi-window-margin-pct))
              (margin (round (* width pct))))
         (set-window-margins win margin nil)))
     nil 'visible)))

;; 3. Hook it so it runs automatically when you split or close windows
(add-hook 'window-configuration-change-hook #'my/update-window-margins)

;; 4. Remove fringes as you preferred in your original config
(fringe-mode 0)

;; 5. Run it once at startup to apply the margins immediately
...
(use-package vertico
  ...
  ;; UI: stop vertico from underlining C-x C-f, M-x
...
(use-package easysession
  ...
  ;; Autosave sessions when changes are made
  ...
  ;; Don't automatically load a session on startup.
  ;; We want to show our launcher screen first.
...
** Org Mode
*** Visuals & Fonts
*** Additional Packages (org-modern, toc-org)
#+begin_src emacs-lisp
(use-package org
  :defer t
  :hook
